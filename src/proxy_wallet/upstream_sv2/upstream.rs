...
pub struct Upstream {
    ...
    job_id: Option<u32>,
    /// Identifier of the job as provided by the ` SetCustomMiningJobSuccess` message
    ...
    // each Downstream instance will add and subtract their hashrate as needed
    ...
    pub fn parse_incoming(self_: Arc<Mutex<Self>>) -> ProxyResult<'static, ()> {
            ...
            .map_err(|_| PoisonLock)?;
        {
            ...
            let start_diff_management = tokio::task::spawn(async move {
                // No need to start diff management immediately
                    ...
                    Ok(SendTo::Respond(message_for_upstream)) => {
                        ...
                        handle_result!(
                            ...
                            tx_frame.send(frame).await.map_err(|e| {
                                crate::error::Error::ChannelErrorSender(
    ...
    pub fn handle_submit(self_: Arc<Mutex<Self>>) -> ProxyResult<'static, ()> {
        ...
        let handle_submit = tokio::task::spawn(async move {
            loop {
                ...
                let frame: StdFrame = handle_result!(tx_status, message.try_into());
                // Doesn't actually send because of Braiins Pool issue that needs to be fixed
